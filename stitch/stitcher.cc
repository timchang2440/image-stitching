...
///*
  ...
  cout << "***************image:size*********** " << imgs.size() << endl;
  cout << imgs[0]._width << " ," << imgs[0]._height << endl;
  cout << imgs[1]._width << " ," << imgs[1]._height << endl;
...
Mat32f Stitcher::build_new() {
  std::cout << "build_new()" << std::endl;
  assign_center();  
  if(LOADHOMO){
      REP(k, (int)imgs.size()){
    	imgs[k].load();
